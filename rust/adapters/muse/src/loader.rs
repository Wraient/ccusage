...
use super::{
    parser::{MuseSession, MuseEntry, parse_session_file, to_loaded_entry},
...
fn load_session_file(
    file_path: &Path,
    shared: &SharedArgs,
) -> (Vec<MuseEntry>, Option<String>) {
    let Ok(contents) = std::fs::read_to_string(file_path) else {
        crate::debug_log(
            shared,
            format!("Failed to read Muse Code session log: {}", file_path.display()),
        );
        return (Vec::new(), None);
    };
