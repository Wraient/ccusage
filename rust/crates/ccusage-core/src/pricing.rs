...
mod tests {
    ...
    fn embedded_pricing_covers_harness_models_for_offline_reports() {
        ...
        // The LiteLLM embed stays family-whitelisted; these harness models
        // price offline through the embedded models.dev catalog instead.
        ...
        assert!(pricing.find("grok-4.5").is_some());
        assert!(pricing.find("grok-4.6").is_some());
        // Muse Code and ZCode models LiteLLM has not published.
        assert!(pricing.find("muse-spark-1.2-contributor").is_some());
        assert!(pricing.find("zai/glm-5.3").is_some() || pricing.find("glm-5.3").is_some());
        // Qwen publishes under Alibaba's DashScope namespace.
